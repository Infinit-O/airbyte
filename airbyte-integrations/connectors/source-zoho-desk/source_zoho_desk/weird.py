from typing import Mapping, Any, MutableMapping, Iterable, Optional

import requests

from .base import ZohoDeskStream

# NOTE: this file contains all the streams that don't conform to the "standard" laid
# out in the Zoho Desk documentation.
# Notably, all of the streams in the file do not honor the "from" or "limit" params, such
# that when those params are passed in, the Stream will raise an exception with error 422.
# This means that a) pagination can't be done on these streams, therefore b) no need for a
# next_page_token. Both methods are overridden.
# Some also place endpoint data in a field other than "data"
...
class Channels(ZohoDeskStream):
    ...
    def request_params(
        ...
        Important note: despite being listed as the general pagination method for the entire API
        certain endpoints do NOT honor the "from" param or the "limit" param and will 422 if it is received.
...
class Modules(ZohoDeskStream):
    ...
    def request_params(
        ...
        """
        # NOTE: despite being listed as the general pagination method for the entire API
        # certain endpoints do NOT honor the "limit" param and will 422 if it is received.
        # This endpoint is one such endpoint, hence request_params is overridden here.
        return {}
