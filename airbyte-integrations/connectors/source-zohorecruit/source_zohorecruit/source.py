...
class SourceZohorecruit(AbstractSource):
    def check_connection(self, logger, config) -> Tuple[bool, any]:
        ...
        try:
            ...
            # NOTE: As long as spec.json is properly configured this branch should never be reached.
            return False, "Exception occurred while accessing config dict. {}".format(e)

        if all([x == "" for x in [client_id, client_secret]]):
            return False, "Neither client_id or client_secret can be empty strings. Check config and try again."

        if any([x == "" for x in [client_id, client_secret]]):
            return False, "Neither client_id or client_secret can be empty strings, Check config and try again."
